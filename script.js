  ...
  function dibujarRadixManual(ascendenteAbs, mcAbs, planetas, mostrarContenido) {
    ...
    if (mostrarContenido) {
      ...
      for (const p of planetasData) {
        ...
        const xInicioExt = CENTRO_X + RADIO_GRADOS * Math.cos(radPlaneta);
        const yInicioExt = CENTRO_Y + RADIO_GRADOS * Math.sin(radPlaneta);
        const xFinExt = CENTRO_X + (RADIO_GRADOS - 10) * Math.cos(radPlaneta);
        const yFinExt = CENTRO_Y + (RADIO_GRADOS - 10) * Math.sin(radPlaneta);
        const lineaPosExt = document.createElementNS("http://www.w3.org/2000/svg", "line");
        lineaPosExt.setAttribute("x1", String(xInicioExt));
        lineaPosExt.setAttribute("y1", String(yInicioExt));
        lineaPosExt.setAttribute("x2", String(xFinExt));
        lineaPosExt.setAttribute("y2", String(yFinExt));
        lineaPosExt.setAttribute("class", "linea-posicion-planeta");
        lienzoSvg.appendChild(lineaPosExt);

        const xInicioInt = CENTRO_X + RADIO_ASPECTOS * Math.cos(radPlaneta);
        const yInicioInt = CENTRO_Y + RADIO_ASPECTOS * Math.sin(radPlaneta);
        const xFinInt = CENTRO_X + (RADIO_ASPECTOS + 10) * Math.cos(radPlaneta);
        const yFinInt = CENTRO_Y + (RADIO_ASPECTOS + 10) * Math.sin(radPlaneta);
        const lineaPosInt = document.createElementNS("http://www.w3.org/2000/svg", "line");
        lineaPosInt.setAttribute("x1", String(xInicioInt));
        lineaPosInt.setAttribute("y1", String(yInicioInt));
        lineaPosInt.setAttribute("x2", String(xFinInt));
        lineaPosInt.setAttribute("y2", String(yFinInt));
        lineaPosInt.setAttribute("class", "linea-posicion-planeta");
        lienzoSvg.appendChild(lineaPosInt);
